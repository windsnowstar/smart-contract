...
contract Context {
    ...
    function _msgSender() internal view returns (identity add1) {
        return msg.sender;
    }
...
contract IERC721 {
     ...
     */
    function balanceOf(identity owner) public view returns (uint256 balance);

    /**
     * @dev Returns the owner of the NFT specified by `tokenId`.
     */
    function ownerOf(uint256 tokenId) public view returns (identity owner);
    ...
    function getApproved(uint256 tokenId) public view returns (identity operator);

    function setApprovalForAll(identity operator, bool _approved) public;
    function isApprovedForAll(identity owner, identity operator) public view returns (bool);


    function safeTransferFrom(identity from, identity to, uint256 tokenId, bytes memory data) public;
}
...
contract IERC721Receiver {
     ...
     */
    function onERC721Received(identity operator, identity from, uint256 tokenId, bytes memory data)
...
library Address {
    ...
    returns (bool addressCheck)
  {
     ...
     * _Available since v2.4.0._
     */
    function toPayable(identity account) internal pure returns (identity ide) {
        return identity(uint160(account));
    }
...
contract ERC721 is Context, ERC165, IERC721 {
    ...
    mapping (uint256 => identity) private _tokenOwner;

    // Mapping from token ID to approved address
    mapping (uint256 => identity) private _tokenApprovals;

    // Mapping from owner to number of owned token
    mapping (identity => Counters.Counter) private _ownedTokensCount;
    ...
    function ownerOf(uint256 tokenId) public view returns (identity) {
        identity owner = _tokenOwner[tokenId];
        require(owner != identity(0), "ERC721: owner query for nonexistent token");
     ...
     */
    function setApprovalForAll(identity to, bool approved) public {
     ...
     * @param _data bytes data to send along with a safe transfer check
     */
    function _safeTransferFrom(identity from, identity to, uint256 tokenId, bytes memory _data) internal {
        _transferFrom(from, to, tokenId);
     ...
     */
    function _safeMint(identity to, uint256 tokenId) internal {
     ...
     * @param _data bytes data to send along with a safe transfer check
     */
    function _safeMint(identity to, uint256 tokenId, bytes memory _data) internal {
        _mint(to, tokenId);
     ...
     */
    function _burn(identity owner, uint256 tokenId) internal {
        require(ownerOf(tokenId) == owner, "ERC721: burn of token that is not own");

        _clearApproval(tokenId);

        _ownedTokensCount[owner].decrement();
        _tokenOwner[tokenId] = identity(0);
    ...
    function _clearApproval(uint256 tokenId) private {
        if (_tokenApprovals[tokenId] != identity(0)) {
            _tokenApprovals[tokenId] = identity(0);
        }
    }
}
...
contract IERC721Enumerable is IERC721 {
    ...
    function tokenOfOwnerByIndex(identity owner, uint256 index) public view returns (uint256 tokenId);
...
contract ERC721Enumerable is Context, ERC165, ERC721, IERC721Enumerable {
    ...
    mapping(identity => uint256[]) private _ownedTokens;
     ...
     */
    function _tokensOfOwner(identity owner) internal view returns (uint256[] storage) {
     ...
     */
    function _addTokenToOwnerEnumeration(identity to, uint256 tokenId) private {
     ...
     */
    function _removeTokenFromOwnerEnumeration(identity from, uint256 tokenId) private {
...
contract ERC721Holder is IERC721Receiver {
    function onERC721Received(identity, identity, uint256, bytes memory) public returns (bytes4) {
        return this.onERC721Received.selector;
    }
}

contract TestAliContract is ERC721Full, ERC721Holder {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor(
        string memory name, //名称
        string memory symbol,//缩写
        string memory baseURI//URI
    ) ERC721Full(name, symbol) public {
        _setBaseURI(baseURI);
    }

    function singleMint(identity player, string memory tokenURI) public returns (uint256) {
        _tokenIds.increment();
